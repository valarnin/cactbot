...
const writeCoverageReport = (
  ...
  const str = `// Auto-generated from ${currentFileName}\n` +
    ...
    `/* eslint-disable */\n` +
    `import { Coverage, CoverageTotals, TranslationTotals, Tags } from './coverage.d';\n\n` +
...
(async () => {
  ...
  writeCoverageReport(currentFileName, outputFileName, coverage, totals, translationTotals, tags);
})().catch((e) => {
  console.error(e);
  process.exit(1);
});
